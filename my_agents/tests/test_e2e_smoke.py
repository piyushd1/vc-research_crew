...
class E2ESmokeTests(unittest.TestCase):
    def test_cli_quick_mode_e2e_smoke(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch("my_agents.controller.CrewAIAgentRunner", return_value=FakeRunner()):
                with patch("my_agents.controller.VCResearchController._load_project_env"):
                    with patch("my_agents.controller.build_llm", return_value=object()):
                        with patch("my_agents.main.VCResearchController") as MockController:
            ...
            with patch("my_agents.controller.CrewAIAgentRunner", return_value=FakeRunner()):
                with patch("my_agents.controller.VCResearchController._load_project_env"):
                    with patch("my_agents.controller.build_llm", return_value=object()):
                        ...
                        with patch("my_agents.controller.VCResearchController.__init__",
                                   lambda self, runner=None, prompt_fn=None, print_fn=None, now_fn=None, project_root_arg=None:
                                   setattr(self, 'runner', FakeRunner()) or
                                   setattr(self, 'prompt_fn', input) or
                                   setattr(self, 'print_fn', print) or
                                   setattr(self, 'now_fn', __import__('datetime').datetime.now) or
                                   setattr(self, 'project_root', project_root)):

                            run([
                                "--company", "SmokeTestCorp",
                                "--focus", "E2E testing",
                                "--verbose"
                            ])
